<?php 
    ...
    $result = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}jne_tracing_api");
    ...
    if ($_POST['submit'] == 'update') {
        ...
        $wpdb->update($wpdb->prefix . 'jne_tracing_api', $formData, ['api_id' => $result->api_id]);
        $status = 1;
    }
        ...
        <div class="jne-form">
          <form action="<?php echo site_url('/wp-admin/admin.php?page=jne-tracing-config'); ?>" method="POST">
            <div class="form-group">
              ...
              <input type="text" name="username" id="username" value="<?php echo $data ? $data->username : ''; ?>"
                required>
            </div>
            <div class="form-group">
            ...
            <div>
              <button class="button button-primary" name="submit"
                value="<?php echo $data ? 'update' : 'submit'; ?>">Save</button>
            </div>
          </form>
        </div>
      </div>
